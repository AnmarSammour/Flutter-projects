...
import 'package:http/http.dart' as http; // هذه المكتبة لا تزال مستخدمة للتوليد العشوائي ومن لون أساسي
...
final paletteGeneratorProvider = StateNotifierProvider<PaletteGeneratorNotifier, AsyncValue<ColorPalette>>((ref ) {
...
class PaletteGeneratorNotifier extends StateNotifier<AsyncValue<ColorPalette>> {
  ...
  Color _colorFromRgb(List<dynamic> rgb) => Color.fromRGBO(rgb[0], rgb[1], rgb[2], 1.0);
  ...
  Future<void> generateRandomPalette({int count = 5}) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<Color> colors = colorsRgb.map((rgb) => _colorFromRgb(rgb)).toList();
        state = AsyncValue.data(ColorPalette(
  ...
  Future<void> generatePaletteFromText(String description, {int count = 5}) async {
    ...
    state = const AsyncValue.loading();
    try {
      final colors = await _aiService.getPaletteFromDescription(description, count);
      state = AsyncValue.data(ColorPalette(
        id: DateTime.now().toIso8601String(),
        colors: colors,
        name: description,
